...
function decodeUnicode(str: string): string {
  return str.replace(/\\u([\da-f]{4})/gi, (_, hex) => String.fromCharCode(parseInt(hex, 16)))
}
...
export default function UnicodePage() {
  ...
  const handleConvert = useCallback(() => {
    ...
    try {
      setOutput(mode === 'encode' ? encodeUnicode(input) : decodeUnicode(input))
    } catch (e) {
  ...
  return (
    <div className="h-full flex flex-col" style={{ padding: 12 }}>
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs font-medium">Unicode 编解码</span>
